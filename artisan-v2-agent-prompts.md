# Artisan v2 — Agent Prompts

Companion to `artisan-v2-spec.md` (§2.6). One system prompt per agent, structured consistently, ready to drop into `engine/artisan/agents/prompts/`.

**Calibration note, read before using these:** these are complete, carefully-structured first drafts, faithful to every schema and guardrail in the main spec — not empirically tuned prompts. That only happens by running them against real shortlist data, reading a sample of actual outputs, and tightening wording wherever behavior drifts from intent. See "Testing & iteration" at the end of this file before treating any of these as final.

---

## Shared conventions

All six prompts follow the same structure and rely on two implementation patterns worth calling out once rather than repeating six times:

**Forced structured output via tool use, not "please respond in JSON."** Each prompt ends by instructing the model to call a dedicated `submit_*` tool. In the actual API call, set `tool_choice` to force that specific tool. This is materially more reliable than asking Claude to emit JSON in prose — the tool's `input_schema` *is* the enforced contract, and a malformed call is a visible, retryable failure rather than a silently-malformed string you have to parse and hope.

**Static text first, per-call data last, for cache efficiency.** Everything above the per-symbol or per-position data payload (role, context description, tools, process, constraints, output format) is identical across every call to a given agent within a run. Structure the actual API request so that block comes first and is marked cacheable, and only the specific symbol's/position's data is appended per call — this is what makes the cost estimate in §2.4 of the main spec hold at scale, since the three per-symbol analysts are called once per shortlisted name every day.

**`historical_precedent` is mandatory everywhere it appears, never optional.** Even for the three narrower analyst agents, a one-line "no relevant history found" is a valid and required answer — the point is forcing the check to happen, not padding the output.

**Model assignment** (from spec §2.2): Haiku 4.5 for Fundamental Analyst, Technical Analyst, Sentiment Analyst, and Briefing. Sonnet 5 for Synthesis and Position Review.

---

## 1. Fundamental Analyst (Haiku 4.5)
...
## 4. Synthesis / Portfolio Manager (Sonnet 5)

The most complex prompt — this is where every hard rule in the spec has to actually be enforced in words, not just implied.
...
Before trusting any of these in production, even paper:

1. **Build a small eval set first** — 10-15 real or reconstructed scenarios (a mix of clean setups, marginal ones, and at least a few that should clearly get downgraded or vetoed) — and run each prompt against it by hand before wiring it into the daily pipeline. This is cheap at Haiku/Sonnet pricing and catches obvious drift early.
2. **Watch the hard-constraint violations specifically**, not just output quality generally — e.g., does Synthesis ever mark "enter" on a symbol outside the eligible set, does it ever reason its way toward more risk when behind the return target, does Position Review ever suggest loosening a stop. These are the cases worth a few explicit few-shot examples in the prompt if you see them slip.
3. **Version every change** (`prompt_version`) and keep the old version's outputs around for comparison — a wording tweak that seems like an improvement on five examples can regress on the sixth.
4. **Re-test after any schema change** in the main spec (new field, changed threshold) — these prompts reference specific field names and values (25% target, 18% tolerance, 8-10 cap, 30-day ceiling) that will drift out of sync with the code if the spec changes and the prompts don't.
