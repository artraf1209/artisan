...
export default async function ShortlistPage({
  ...
  const factorRows = ((factorRowsData.data ?? []) as Array<Record<string, unknown>>).map((row) => ({
    ...
    <>
      {strategies.length > 1 ? (
        <section className="rounded-[1.5rem] border border-border bg-card/95 p-4 shadow-[0_20px_45px_rgba(0,0,0,0.22)]">
          <form action="/recommendations/shortlist" className="flex flex-wrap items-end gap-3">
